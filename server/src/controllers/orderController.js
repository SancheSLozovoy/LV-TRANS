...
import * as MetricModel from '../models/metricsModel.js';
...
export async function getActiveOrdersByUserId(req, res) {
    const userId = Number(req.params.userId);
    const currentUserId = req.user.id;
    ...
    try {
        const { orders, total } = await OrderModel.getActiveOrdersByUserId(
            userId,
            limit,
            offset,
        );
...
export async function getDashboardMetrics(req, res) {
    try {
        ...
        const [
            ...
            safeGet(MetricModel.getCargoAnalytics, 'аналитика по грузу'),
            safeGet(MetricModel.getTemporalAnalytics, 'временная аналитика'),
            safeGet(MetricModel.getStatusAnalytics, 'аналитика по статусам'),
            safeGet(MetricModel.getBusinessKPI, 'бизнес-показатели'),
            safeGet(MetricModel.getComplexMetrics, 'сложные метрики'),
        ]);
        ...
        console.error('Ошибка в getDashboardMetrics:', err);
